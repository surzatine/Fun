// Generate random prize number (1-13)
var num = Math.floor(Math.random() * 13) + 1;

// Initialize the scratch card
$(document).ready(function() {
    $("#card").wScratchPad({
        size: 80, // Brush size for scratching
        bg: createPrizeBackground(num), // Background with prize
        fg: createScratchSurface(), // Silver scratch surface
        ...
        // Callback when scratching completes
        scratchUp: function(e, percent) {
            // When 50% or more is scratched, reveal the full prize
            if (percent > 50) {
                this.clear();
                celebratePrize();
            }
...
// Create prize background
function createPrizeBackground(prizeNum) {
    // Create a canvas to draw the prize
    ...
    ctx.beginPath();
    ctx.roundRect(50, 100, 350, 300, 20);
    ctx.fill();
    ...
    ctx.fillText('🎉 YOU WON! 🎉', 225, 180);
    
    // Prize number
    ctx.fillStyle = '#667eea';
    ctx.font = 'bold 100px Poppins, sans-serif';
    ctx.fillText('Prize #' + prizeNum, 225, 300);
    
    // Congratulations text
    ctx.fillStyle = '#718096';
    ctx.font = '600 24px Poppins, sans-serif';
    ctx.fillText('Congratulations!', 225, 360);
